...
private:
    ...
    void step_7_check_tray_empty_position()
    {
        ...
        // Switch to Camera 1 (Output Tray) - early for optimization
        if (!switchAndWaitForCameraWithRetry(1, 3))
        {
            RCLCPP_ERROR(this->get_logger(), "[STEP 7] ❌ Camera 1 switch failed - retrying step");
            // Stay in current step to retry
            return;
        }
    ...
    void check_cartridge_on_tray_empty()
    {
        ...
        // Switch to Camera 0 (Input Tray) for detection
        if (!switchAndWaitForCameraWithRetry(0, 3))
        {
            RCLCPP_ERROR(this->get_logger(), "[AI check] ❌ Camera 0 switch failed - aborting step");
            // Retry this step
            return;
        }
