...
public:
    CSIDualCameraNode() : Node("csi_dual_camera_node"), running_(true)
    {
        ...
        if (cam0_enable_) {
            ...
            RCLCPP_INFO(get_logger(), "⏭️  CAM0 is disabled.");
        }
        ...
        if (cam1_enable_) {
            ...
            RCLCPP_INFO(get_logger(), "⏭️  CAM1 is disabled.");
        }
...
private:
    ...
    CamProcess launch_rpicam(int cam_id, int width, int height, int fps)
    {
        ...
        if (pid == 0) {
            ...
            //   --ev -0.5            : Exposure compensation -0.5 for optimal tray and cartridge visibility
    ...
    {
        if (cam_id == 0 && !cam0_enable_) {
            RCLCPP_INFO(get_logger(), "⏭️  CAM0 is disabled. Capture thread exiting.");
            return;
        }
        if (cam_id == 1 && !cam1_enable_) {
            RCLCPP_INFO(get_logger(), "⏭️  CAM1 is disabled. Capture thread exiting.");
            return;
        }
                ...
                if (!got_frame) {
                    ...
                    if (stall_duration > std::chrono::seconds(8)) {
                        ...
                        if (cfd >= 0) {
                            ...
                            dprintf(cfd, "Param Mode   : 2028:1520:12:P\n");
