...
class CartridgeSystem(Node):
    ...
    def process_state(self):
        ...
        # ── MOTION DELAY: non-blocking 2s wait between motions ──────────────
            ...
            # InX di chuyển đến cuối băng tải — non-blocking, sau đó check S3 trong S1_INX_WAIT_STOP
            ...
            if self.is_iny_safe_for_inx_move():
                ...
                self.get_logger().info(f"▶️ InX moving to {self.config.inx_target2:.0f}mm (non-blocking) — monitoring S3")
                self._inx_s3_wait_start = None  # reset 30s timer
                self._inx_arrived = False
                self.clear_guide()
                self.state = SystemState.S1_INX_WAIT_STOP
                self.log_guide_once("S1_INX_MOVING", "[S1] InX đang di chuyển — chờ S3 ON hoặc InX đến nơi")
            else:
        ...
        # ══════════════════════════════════════════════════════════════
            ...
            if self.move_servo(2, target_pos):
                ...
                self.log_guide_once("S2B_WAIT_S11", "[S2-B] Cylinder 1 đang extend (kẹp khay mới) → Đợi S11 ON (Cyl1 MAX). Kích: '11:1'")
            else:
                self.state = SystemState.ERROR
            ...
            if self.move_servo(2, self.config.iny_target2):
                ...
                self.log_guide_once("S2B_WAIT_S10", "[S2-B] Cylinder 1 đang retract (thả khay mới) → Đợi S10 ON (Cyl1 MIN). Kích: '10:1'")
            else:
                self.state = SystemState.ERROR
